...
>
> - **読み取り**: FSx S3 AP 上の全クエリ成功（10K / 5M 行 Parquet）
> - **書き戻し (CTAS)**: ファイルベースメタストアの制限により失敗（FSx S3 AP の問題ではない）
...
Trino（オープンソース分散 SQL クエリエンジン）を使用して FSx for ONTAP のデータを S3 Access Points 経由でクエリします。Trino の S3 ファイルシステム実装はパススタイルアクセスをサポートしており、FSx S3 AP エイリアスと互換性があります。
...
Trino は構造化データの分散 SQL クエリに特化していますが、非構造化データのメタデータクエリに活用できます。

**パターン:**
1. **メタデータテーブル** — ファイルパス・サイズ・タイプを Hive テーブルとして登録しクエリ
2. **マルチソースフェデレーション** — FSx S3 AP のファイルカタログと他のデータソース（RDS, PostgreSQL）を JOIN
