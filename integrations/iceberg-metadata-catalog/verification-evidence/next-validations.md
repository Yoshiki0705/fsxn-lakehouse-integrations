...
| C-11 | ETL S3 Tables → standard Glue Iceberg → Snowflake VENDED_CREDENTIALS | Medium | ❌ **Access Denied** (2026-06-03): ETL to standard Glue Iceberg table succeeded (PyIceberg write + Athena query confirmed). However, Snowflake VENDED_CREDENTIALS on standard Glue catalog also returns "Access is not allowed" — same error as S3 Tables path. Manual curl test with assumed role confirms Glue REST returns 403 on loadTable. Likely requires public preview opt-in or additional Lake Formation Application Integration configuration. AWS support follow-up needed. |
