...
| 制限事項 | ゲノミクスへの影響 |
|---------|-----------------|
| S3 AP（パイプラインは読み取りのみ使用） | カタログ経由で再解析パイプラインをトリガー不可 |
...
- **バイナリファイル形式**: FASTQ、BAM、VCF などのバイオインフォマティクス形式は Bedrock Claude で直接分類できません。AI 分類前にフォーマット固有パーサー（例: BioPython）を前処理 Lambda で使用し、ヘッダーと品質メトリクスを抽出してください。[AI プロンプトガイド](ai-prompt-customization-guide-ja.md)のマルチモーダルマトリクスを参照。
