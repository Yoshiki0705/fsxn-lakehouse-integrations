...
def generate_transactions(output_dir: Path, num_rows: int = 500_000) -> None:
    ...
    print(f"  Generating transactions ({num_rows:,} rows)...")
    ...
    for chunk_start in range(0, num_rows, chunk_size):
        ...
        for (year, month), group in df.groupby(["year", "month"]):
            partition_dir = output_dir / "transactions" / f"year={year}" / f"month={month}"
    ...
    print(f"  ✅ transactions: {num_rows:,} rows written (partitioned by year/month)")


def generate_customers(output_dir: Path, num_rows: int = 50_000) -> None:
    """Generate CSV customers dataset."""
    print(f"  Generating customers ({num_rows:,} rows)...")
...
def generate_events(output_dir: Path, num_rows: int = 100_000) -> None:
    ...
    print(f"  ✅ events: {num_rows:,} rows written (JSON Lines)")


def main():
    parser = argparse.ArgumentParser(description="Generate sample data for Athena verification")
