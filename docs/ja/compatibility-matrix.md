...
| **Databricks** | 非構造化（画像/動画/文書/音声） | `FILE MANAGED` 列（FILE 型、β） | ⚠️ 利用可能だがコピーを伴う | UC マネージド Volume を FileSpace として使用 | 動作するが、バイト列を UC 管理ストレージへコピーする代償を伴う。zero-copy と ONTAP の効率機能を失い、オブジェクトタグも読めなくなる（Databricks 管理ストレージでは `null`）。β では自動ガベージコレクションなし。本リポジトリでは未実行 |
| **Databricks** | 任意のファイル | `_object_metadata` のオブジェクトタグ / ユーザーメタデータ（DBR 18.2+） | 🔲 未検証 | — | オブジェクトストレージのメタデータをテーブル列へ橋渡しする公式手段。オブジェクトタグ付けは FSx for ONTAP S3 AP で**サポートされる**（[2026-08-12 検証](../../verification-pack/s3ap-object-tagging/evidence/2026-08-12/evidence-record.yaml)）が、Databricks が AP パス経由でタグを読めるかは未検証。ケース `DBX-FILE-041` |
