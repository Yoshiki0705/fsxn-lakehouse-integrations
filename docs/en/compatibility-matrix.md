...
| **Databricks** | Unstructured (image/video/doc/audio) | `FILE MANAGED` column (FILE type, Beta) | ⚠️ Available, but copies | UC managed volume as FileSpace | Works, at the cost of copying bytes into UC-managed storage — zero-copy and ONTAP efficiencies are forfeited, and object tags become unreadable (`null` for Databricks-managed storage). Beta has no automatic garbage collection. Not yet run in this repository |
| **Databricks** | Any file | `_object_metadata` object tags / user metadata (DBR 18.2+) | 🔲 Unverified | — | The documented bridge from object-storage metadata to table columns. Object tagging **is** supported on an FSx for ONTAP S3 AP ([verified 2026-08-12](../../verification-pack/s3ap-object-tagging/evidence/2026-08-12/evidence-record.yaml)), but whether Databricks can read tags through an AP path is untested. Case `DBX-FILE-041` |
