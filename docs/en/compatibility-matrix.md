...
| **Snowflake** | Iceberg | Write (Managed Iceberg Table) | ⚠️ Design documented, end-to-end not verified | COPY INTO from FSx for ONTAP S3 AP External Stage → Managed Iceberg Table on customer S3 | **Verified**: External Volume creation (`s3tables_metadata_vol`, 2026-05-31) and COPY INTO from an AP-backed stage into a standard table (2026-05-24). **Not verified**: the Managed Iceberg write end to end, Dynamic Table as a source (FULL refresh / TARGET_LAG behaviour), COPY INTO 64-day deduplication, and Horizon Catalog governance enforcement for external engines. Treat as a design to validate in your own environment before production use. |
