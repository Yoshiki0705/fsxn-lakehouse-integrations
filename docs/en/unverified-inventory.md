...
| UNV-028 | Databricks | Whether a `FILE MANAGED` FileSpace may be an **external** volume | **Partly closed 2026-08-12**: table creation for both `FILE MANAGED` and `FILE EXTERNAL` is verified on a serverless SQL warehouse, and a FileSpace need not be a dedicated volume. The external-volume question needs a storage credential, which a trial workspace lacks. [Evidence](../../verification-pack/databricks/file-type/evidence/2026-08-12/evidence-record.yaml) |
