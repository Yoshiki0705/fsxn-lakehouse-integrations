...
| Claiming "S3 fully compatible" | FSx for ONTAP S3 AP supports a subset of S3 operations. No Object Versioning, no conditional writes, no presigned URLs, 50 GB upload limit | Use precise language: "S3 API access for supported operations" with link to compatibility matrix |
